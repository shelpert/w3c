...
%
    ...
    properties
        ...
        N_es=0; % early stopping count of eps < EPS_es. If N_es == 0 then don't run early stopping
        EPS_es = 1e-15; % early stopping epsilon for N_es values < EPS_es
    ...
    methods
        ...
        function obj = HeunSolver(obj,X_0)
            ...
            for tidx=2:obj.Nt
                ...
                added_terms = obj.dt/2*(dX + obj.D(obj.t(tidx),slntmp)) ... drift term (second order approx)
                            + obj.sigma*sqrt(obj.dt)*noise; % diffusion term
                sln(:,tidx) = sln(:,tidx-1) + added_terms;

                % --- step 3: if added_terms < EPS_es for N_es times, do
                % stop early to avoid useless computation
                if obj.N_es > 0 && all(abs(added_terms) < obj.EPS_es)
                    cnt_es = cnt_es + 1;
                    if cnt_es > obj.N_es
                        break;
                    end
